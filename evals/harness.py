"""Per-case factual checks and retrieval experiments."""
from __future__ import annotations

import json
import time

from perk_watch.runtime.agent import EvidenceSelection, _render, answer_with_db
...
def run_case(db, case: dict, *, client, embedder, model: str, api_usage: dict) -> dict:
    captured: list[dict] = []
    ...
    answer = answer_with_db(db, case["question"], client=PlannedAgentClient(tool_plan(case)), model=model,
                            embedder=embedder, on_tool_result=lambda tool, result: captured.append({"tool": tool, "result": result}),
                            metrics=metrics)
    planned_failures = factual_checks(case.get("expected", {}), captured, answer)
    live_captured: list[dict] = []
    live_metrics: dict[str, int] = {}
    live_started = time.perf_counter()
    live_answer = answer_with_db(db, case["question"], client=client, model=model, embedder=embedder,
    ...
    return {"id": case["id"], "question": case["question"], "tool_results": live_captured,
